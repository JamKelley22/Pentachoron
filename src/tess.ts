...
type BtnTypes = "convert" | "cancel" | "reset" | "canceling" | "none";
...
function setFelix() {
  let felix = <HTMLImageElement>document.getElementById("logo");
...
function escapeSpacesInFilepath(userFilePath: string): string {
...
async function start(pdfPath: string) {
  dLog("start");
  ...
  if (split.length > 1) {
    ...
    switch (extention) {
      case "PDF":
        ...
        try {
          if (checkCancel()) return;
          imageNamesArr = await getImageFilenames();
        } catch (err) {
          error(err);
        }
        try {
          if (checkCancel()) return;
          await tesseract(imageNamesArr);
          await combineTextFiles(mergedTextOutputName);
        } catch (err) {
          error(err);
        }
    ...
    shell.showItemInFolder(
      path.join(__dirname, ...["output", mergedTextOutputName])
    );
...
let secRemaining = 0;
async function tesseract(imageNamesArr: string[]) {
  dLog("tesseract");
  if (checkCancel()) return;
  log("Scanning Images");
  setProgress(0);
  ...
  document.getElementById(
    "writeLocation"
  ).innerHTML = `Files being written to ${outputFolder}`;
  let cnt = 1;
  ...
  for (let imageName of imageNamesArr) {
    ...
    let ret = await decodeImage(imageName, outputTextFile, cnt);
...
function updateTimeRemaining() {
  ...
  if (secRemaining > 60) {
    document.getElementById(
      "timeRemaining"
    ).innerHTML = `Estimated Time Remaining ${Math.floor(
      secRemaining / 60
    )} minutes ${Math.round(secRemaining % 60)} seconds`;
  } else if (secRemaining > 1) {
    document.getElementById(
      "timeRemaining"
    ).innerHTML = `Estimated Time Remaining ${Math.round(
      secRemaining
    )} seconds`;
  } else {
    document.getElementById(
      "timeRemaining"
    ).innerHTML = `Estimating Time Remaining`;
  }
}
  ...
  for (let file of text_files) {
    ...
    try {
      let fileContents = fs.readFileSync(
        path.join(__dirname, ...["output", "text", file])
      );
...
function getImageFilenames(): Promise<string[]> {
...
function setVisualFilePath(path: string) {
...
function setProgress(progress) {
  ...
  (<HTMLProgressElement>document.getElementById("progressBar")).value =
    progress * 100;
  document.getElementById("progressPercent").innerHTML = `${Math.round(
    progress * 100
  )}%`;
}
...
function setConvertFrom() {
  dLog("setConvertFrom");
  convertFrom = (<HTMLSelectElement>document.getElementById("convertFrom"))
    .value;
  if (convertFrom !== "") {
...
function setConvertTo() {
  dLog("setConvertTo");
  convertTo = (<HTMLSelectElement>document.getElementById("convertTo")).value;
  if (convertTo !== "") {
  } else {
  }
  checkVisibleButton();
...
function setModalBtn(btn: BtnTypes) {
  dLog("setModalBtn");
  dLog(btn);
  switch (btn) {
    case "convert":
