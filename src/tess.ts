...
async function decodeImage(imageName:string, outputTextFile:string,num:number) {
	dLog("decodeImage")
	if (checkCancel()) return;
	return new Promise(function(resolve, reject) {
...
function setProgress(progress) {
	...
	(<HTMLProgressElement>document.getElementById("progressBar")).value = progress * 100;
	document.getElementById("progressPercent").innerHTML = `${Math.round(progress * 100)}%`
}

function setConvertFrom() {
	dLog("setConvertFrom")
	convertFrom = (<HTMLSelectElement>document.getElementById("convertFrom")).value;
	if(convertFrom !== '') {
